...
public class Controller implements Initializable {
    ...
    public void sauvegardeEtape() {
        ...
        try {
            ...
            monFichier = new FileWriter("./src/ressources/dataEtape.txt");
            monFichier.write("Titre du livre :" + this.livre.getName() + "\n");
            if(this.livre.getListeEtapes().size()>0 && this.circlesOnScreen.size()>0) {
                for (int i = 0; i < this.livre.getListeEtapes().size(); i++) {
                    monFichier.write("Numero de l'etape :" + i + "\n" +
                            this.livre.getEtape(i).getTexte() + "\n" +
                            "µµµµµµµ" + "\n" +
                            "Coord X : " + (this.circlesOnScreen.get(i).centerXProperty().get()) + "\n" +
                            "Coord Y : " + (this.circlesOnScreen.get(i).centerYProperty().get()) + "\n" +
                            "§§§§§§§" + "\n");


                }

                monFichier.write("Premiere Etape :" + this.livre.getNumEtape(this.livre.getFirstEtape()) + "\n" +
                        "Dernières Etape : ");
                for (Etape etape : this.livre.getEtapeFinal()) {
                    monFichier.write("\n" + this.livre.getNumEtape(etape));
                }
                monFichier.write("\n" + "~~~~~~~" + "\n" + "~~~~~~~");
            }
            else{
    ...
    public void sauvegardeInventaire(){
        ...
        try {
            // Ouvre le fichier xyz.dat et y enregistre
            // les données du tableau
            monFichier = new FileWriter("./src/ressources/dataInventaire.txt");
            if (this.inventaire.getListeEditeur().keySet().size() > 0) {
                for (String affiche : this.inventaire.getListeEditeur().keySet()) {
                    ...
                    if (this.inventaire.getListeEditeur().containsKey(affiche)) {
                        ...
                        monFichier.write("Nom de l'objet : " + affiche + "\n"
                                + "Etape d'aquisition : " + aqui + "\n" +
                                "Etape d'utilisation : " + uti + "\n");
                        if (carre != null) {
                            monFichier.write("Coord X :" + carre.getX() + "\n" +
                                    "Coord Y :" + carre.getY() + "\n");
                        }
                    }
                }
    ...
    public void backup() {
        ...
        try {
            ...
            monFichier = new FileReader("./src/ressources/dataEtape.txt"); // On ouvre le data.txt
            ...
            if (ligne == null ){
                ...
                monFichier = new FileReader("./src/ressources/dataEtape.txt");
    ...
    public void backupInventaire(){
        ...
        try {
            // Lecture du fichier pour création des étapes / récupération du contenu
            monFichier = new FileReader("./src/ressources/dataInventaire.txt"); // On ouvre le data.txt
            tampon = new BufferedReader(monFichier); // On stocke le contenu dans le buffer
            String ligne = tampon.readLine(); // On lis les lignes
            MySquare square;
            double x;
